...
function render(){
 ...
 const badge=document.getElementById('alerts-badge');badge.textContent=data.actionCount;badge.hidden=!data.actionCount;
 ...
 const focus=new URL(location.href).searchParams.get('watch');if(focus){const el=document.getElementById('watch-'+focus);if(el)el.open=true;}
}
