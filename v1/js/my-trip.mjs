...
export function createMyTrip({request,getToken,showPage,openParties,icon,onHomeData=()=>{}}) {
  ...
  function planCard(p) {
    ...
    const directionsLocation=directionsLocationForPlan(p), directionsHref=directionsUrl(directionsLocation);
    const directions=directionsHref?`<a href="${esc(directionsHref)}" target="_blank" rel="noopener noreferrer" data-trip-action="directions" data-id="${esc(p.id)}">Directions</a>`:'';
  ...
  root.addEventListener('click',async event=>{
    ...
    try {
      ...
      if(action==='directions'){event.preventDefault();const plan=data.itineraries[t.id].find(p=>p.id===b.dataset.id);if(plan)openDirections(directionsLocationForPlan(plan));}
